...
import (
	...
	"github.com/dtegapp/mochi-mqtt/v2/hooks/storage"
...
type Options struct {
	...
	//
	// 켜면 발행 1건마다 페이로드 크기만큼의 버퍼 할당과 memcpy가 사라진다 — 대용량 응답
	// (스냅샷·LiveList 수백KB~MB)이나 구독자가 여럿인 브로드캐스트 토픽에서 특히 크다
	// (구독자 N명이면 상류는 페이로드를 N번 복사한다).
	//
	// 대가: 쓰기가 1회에서 2회로 나뉜다. TCP에서는 writev 한 번으로 합쳐지지만,
	// TLS·WebSocket 위에서는 레코드/프레임이 2개로 나뉜다(MQTT는 제어패킷이 프레임 경계와
	// 일치하지 않아도 되므로 프로토콜상 문제는 없고, 오버헤드는 패킷당 수~수십 바이트다).
	// 회선에 나가는 바이트 순서·내용은 켜든 끄든 동일하다.
	SplitPublishWrite bool `yaml:"split_publish_write" json:"split_publish_write"`
}
