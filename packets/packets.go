...
import (
	...
	"github.com/dtegapp/mochi-mqtt/v2/mempool"
)
...
// PublishEncodeHeader는 PublishEncode와 동일한 바이트를 만들되 Payload는 쓰지 않는다.
// 헤더만 버퍼에 담고 페이로드는 호출부가 별도로 내보내게 해서, 페이로드 크기만큼의
// 버퍼 할당과 memcpy를 없애기 위한 것이다(Client.WritePacket 참고).
//
// D-TEG 20260821: 512KB 스냅샷 1건 발행에서 PublishEncode가 페이로드를 버퍼로 복사하며
// 페이로드 크기만큼을 한 번 더 할당하고 있었다(발행 1건당 메모리 2배). 상류 동작을
// 바꾸지 않기 위해 PublishEncode는 그대로 두고 이 함수를 추가한다.
func (pk *Packet) PublishEncodeHeader(buf *bytes.Buffer) error {
	nb := mempool.GetBuffer()
	defer mempool.PutBuffer(nb)
