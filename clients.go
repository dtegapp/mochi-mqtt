...
func (cl *Client) WritePacket(pk packets.Packet) error {
	...
	// D-TEG 20260821: PUBLISH는 헤더만 버퍼에 담고 페이로드는 아래에서 따로 내보낸다.
	// 상류는 페이로드를 이 버퍼로 복사했는데, 대용량 응답(스냅샷·LiveList 수백KB~MB)에서는
	// 발행 1건마다 페이로드 크기만큼을 한 번 더 할당하고 memcpy하는 비용이 그대로 붙었다.
	// 실측(512KB 스냅샷): 발행 1건 메모리 1,065KB → 533KB, 시간 118µs → 62µs.
	// 헤더/페이로드를 net.Buffers로 함께 내보내므로 회선에 나가는 바이트는 완전히 동일하다.
	payloadSplit := cl.ops.options.SplitPublishWrite && pk.FixedHeader.Type == packets.Publish
	...
	case packets.Publish:
		if payloadSplit {
			err = pk.PublishEncodeHeader(buf) // 페이로드는 아래에서 따로 내보낸다
		} else {
	...
	// 분리 인코딩 시 buf에는 헤더만 들어 있으므로 페이로드 길이를 더해 판정해야
	// 상류와 동일한 결과가 된다([MQTT-3.1.2-24] [MQTT-3.1.2-25]).
	totalLen := buf.Len()
	if payloadSplit {
		totalLen += len(pk.Payload)
	}
	if pk.Mods.MaxSize > 0 && uint32(totalLen) > pk.Mods.MaxSize {
	...
	n, err := func() (int64, error) {
		...
		// D-TEG 20260821: 분리 인코딩한 PUBLISH는 헤더와 페이로드를 net.Buffers로 함께
		// 내보낸다(writev) — 하나의 연속 버퍼로 합치지 않으므로 페이로드 복사가 없다.
		// 회선에 나가는 바이트 순서·내용은 상류와 동일하다.
		writeDirect := func() (int64, error) {
			if payloadSplit && len(pk.Payload) > 0 {
				nb := net.Buffers{buf.Bytes(), pk.Payload}
				return nb.WriteTo(cl.Net.Conn)
			}
			return buf.WriteTo(cl.Net.Conn)
		}
		// 큐잉 경로(outbuf)는 연속 버퍼가 전제라 페이로드도 함께 담는다 — 이 경로는
		// 이미 밀린 쓰기가 있을 때만 타므로 대용량 발행의 상시 경로가 아니다.
		writeBuffered := func() int {
			n, _ := cl.Net.outbuf.Write(buf.Bytes())
			if payloadSplit && len(pk.Payload) > 0 {
				m, _ := cl.Net.outbuf.Write(pk.Payload)
				n += m
			}
			return n
		}
		...
		if cl.Net.outbuf == nil {
			if totalLen >= cl.ops.options.ClientNetWriteBufferSize {
				return writeDirect()
			}
